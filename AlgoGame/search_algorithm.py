...
def dfs(board, start, goal):
    ...
    while stack:
        ...
        for direction in ["up", "right", "down", "left"]:  # Other orders are fine too.
            row_offset, col_offset = config.offset_parameter[direction]
            neighbour = (current[0] + row_offset, current[1] + col_offset)
            if helpers.is_legal_position(board, neighbour) and neighbour not in visited:
...
def astar(board, start_pos, goal_pos):
    pq = PriorityQueue()
    pq.put(start_pos, 0)
    g_values = {}
    ...
    while not pq.is_empty():
        ...
        for direction in ["up", "right", "down", "left"]:
            ...
            if helpers.is_legal_position(board, neighbour):
